...
def test_rider_register_and_login_flow(driver):
    """Registers a unique rider from UI, then logs in and verifies redirect + token."""
    ...
    driver.get(f"{FRONTEND_URL}/rider/register")
    ...
    wait.until(
        lambda d: "/rider/login" in d.current_url
    )
    ...
    wait.until(
        lambda d: "/rider/dashboard" in d.current_url
    )
